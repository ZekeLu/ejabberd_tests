...
init_per_testcase(CaseName, Config0) ->
    ...
    ejabberd_config_file_utils:backup_config(Config1),
    run_config_file_modification_fun(Config1),
    Config1.

end_per_testcase(CaseName, Config) ->
    escalus:end_per_testcase(CaseName, Config),
    ejabberd_config_file_utils:restore_config(Config),
...
restart_ejabberd_node() ->
    ok = ejabberd_node_utils:call_fun(application, stop, [ejabberd]),
    ok = ejabberd_node_utils:call_fun(application, start, [ejabberd]).

reload_through_ctl(Config) ->
    OutputStr = ejabberd_node_utils:call_ctl(Config, reload_local),
